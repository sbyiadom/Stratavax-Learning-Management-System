...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get unique filter options - Fixed variable names
    ...
    if (coursesData) {
      coursesData.forEach(item => {
        if (item.course_name && !availableCourses.includes(item.course_name)) {
          availableCourses.push(item.course_name)
        }
      })
      availableCourses.sort()
    }
    
    // Build unique departments list
    const availableDepartments: string[] = []
    if (departmentsData) {
      departmentsData.forEach(item => {
        if (item.department && !availableDepartments.includes(item.department)) {
          availableDepartments.push(item.department)
        }
      })
    ...
    return NextResponse.json({
      ...
      filters: { courses: availableCourses, departments: availableDepartments }
    })
